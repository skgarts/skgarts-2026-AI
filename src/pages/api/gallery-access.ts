...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // Elevate the specific query function (same shape as the working upload route:
    // auth.elevate(files.generateFileUploadUrl)). We elevate items.query itself,
    // then build the query chain on the elevated function's result.
    const elevatedQuery = auth.elevate(items.query);

    // Look up by slug first, then by _id.
    let found: any = null;
    if (slug) {
      const bySlug = await elevatedQuery(COLLECTION).eq('slug', slug).limit(1).find();
      found = bySlug.items?.[0] || null;
    }
    if (!found) {
      const key = slug || id;
      const byId = await elevatedQuery(COLLECTION).eq('_id', key).limit(1).find();
      found = byId.items?.[0] || null;
    }

    const gallery: any = found;
